...
private:
	...
	void draw_adc()
	{
		uint16_t volts = heating.vin / 1000;
		uint16_t mvolts = heating.vin % 1000;
		sprintf(buffer, "Vin:%2d.%03dV", (int)volts, (int)mvolts);
		x = fb.draw_text(0, 0, buffer, Font::sans8);
		sprintf(buffer, "I1 :%4dmA", (int)heating.i1);
		x = fb.draw_text(0, 10, buffer, Font::sans8);
		sprintf(buffer, "I2 :%4dmA", (int)heating.i2);
		x = fb.draw_text(0, 20, buffer, Font::sans8);
		volts = heating.ttip / 100;
		mvolts = heating.ttip % 100;
		sprintf(buffer, "Ttip:%3d.%02dC",(int)volts, (int)mvolts);
